...
export function Card({ className, asset = true, assetType = 'Icon', body = 'Body text for whatever you would like to say.', button = true, children, direction = 'Horizontal', heading = 'Title', icon, variant = 'Stroke' }: CardProps) {
  return <article className={`sds-card sds-card--${direction.toLowerCase()} sds-card--${variant.toLowerCase()} ${className ?? ''}`}>
    ...
    <div className="sds-card__body">
      ...
      {children}
      {button && <Button label="Button" variant="Primary" size="Medium" state="Default" />}
    </div>
  </article>;
}